...
interface FormState {
  id: number | null; name: string; description: string; overview: string; price: number; type: string; 
  access_tier: string; quantity: number; is_active: boolean; features: any[]; faqs: any[]; changelogs: any[];
}

const defaultForm: FormState = {
  id: null, name: '', description: '', overview: '', price: 0, type: '', access_tier: '', quantity: 0, 
...
const filteredProducts = computed(() => {
  const currentType = route.meta.productType as string;
  if (!currentType) return products.value;
  return products.value.filter(p => p.product_type === currentType);
});

const pageTitle = computed(() => {
  if (route.meta.productType === 'software') return 'Software & Layanan';
  if (route.meta.productType === 'digital') return 'Katalog Produk Digital';
  if (route.meta.productType === 'physical') return 'Toko Produk Fisik';
  return 'Manajemen Produk';
});

// FITUR BARU: Menyaring masterTiers agar hanya menampilkan paket Gold
const goldTiers = computed(() => {
  return masterTiers.value.filter(t => t.slug.toLowerCase().includes('gold'));
});
...
const fetchTiers = async () => {
  ...
  } catch (error) { console.error('Gagal memuat master tiers:', error); }
};
...
// Mengisi harga secara otomatis saat dropdown tier dipilih
const autoFillPrice = (tierSlug: string) => {
  if (tierSlug === 'all' || !tierSlug) { form.value.price = 0; return; }
  const selectedTier = masterTiers.value.find(t => t.slug === tierSlug);
  if (selectedTier) form.value.price = Number(selectedTier.price);
};
...
const handleTypeChange = () => {
  ...
  if (form.value.type === 'Software') {
    form.value.access_tier = ''; // Biarkan kosong agar admin memilih varian Gold yang tepat
...
const handleTierChange = () => { autoFillPrice(form.value.access_tier); };
...
const openModal = () => {
  ...
  if (route.meta.formType) {
    form.value.type = route.meta.formType as string;
    handleTypeChange();
  }
...
const closeModal = () => { isModalOpen.value = false; resetImageState(); };
...
const handleMainImageUpload = (event: any) => {
  ...
  if (file) {
    imageFile.value = file; mainImagePreview.value = URL.createObjectURL(file); removeMainImageFlag.value = false;
  }
};
const removeMainImage = () => { imageFile.value = null; mainImagePreview.value = null; existingMainImage.value = null; removeMainImageFlag.value = true; };
const handleScreenshotsUpload = (event: any) => {
  const files = Array.from(event.target.files) as File[];
  files.forEach(file => { screenshotFiles.value.push(file); newScreenshots.value.push({ file, preview: URL.createObjectURL(file) }); });
};
const removeNewScreenshot = (idx: number) => { newScreenshots.value.splice(idx, 1); screenshotFiles.value.splice(idx, 1); };
const removeExistingScreenshot = (id: number, idx: number) => { deletedScreenshots.value.push(id); existingScreenshots.value.splice(idx, 1); };
...
const submitProduct = async () => {
  ...
  try {
    if (isEditMode.value && form.value.id) {
      ...
      await api.post(`/admin/products/${form.value.id}`, formData, { headers: { 'Content-Type': 'multipart/form-data' } });
    } else {
      await api.post(`/admin/products`, formData, { headers: { 'Content-Type': 'multipart/form-data' } });
    }
    closeModal(); fetchProducts(); alert('Produk berhasil disimpan!');
  } catch (error: any) {
  ...
  <div class="p-6 md:p-10 flex-1 max-h-screen overflow-y-auto">
    <div class="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
      <div>
        ...
        <p class="text-sm text-slate-500 mt-1">Kelola dan update katalog {{ pageTitle.toLowerCase() }} Anda di sini.</p>
      </div>
        ...
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="bg-slate-50 border-b border-slate-200 text-slate-500 text-xs uppercase tracking-wider">
              ...
              <th class="p-4 font-black">Syarat Akses</th>
              <th class="p-4 font-black text-center">Status</th>
              <th class="p-4 font-black text-center">Aksi</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-slate-100">
            <tr v-if="loading"><td colspan="5" class="p-8 text-center text-slate-400">Memuat data...</td></tr>
            <tr v-else-if="filteredProducts.length === 0"><td colspan="5" class="p-8 text-center text-slate-400">Belum ada data di menu ini.</td></tr>
            
            <tr v-else v-for="product in filteredProducts" :key="product.id" class="hover:bg-slate-50/50 transition">
              <td class="p-4">
              ...
              <td class="p-4">
                ...
                <div class="text-[11px] font-black text-indigo-600">Rp {{ Number(product.price).toLocaleString('id-ID') }}</div>
              </td>
              <td class="p-4">
                <span class="px-2.5 py-1 rounded-md text-[10px] font-black uppercase"
                      :class="{
                        'bg-amber-100 text-amber-700': product.access_tier?.includes('gold'),
                        'bg-slate-200 text-slate-700': product.access_tier?.includes('silver'),
                        'bg-blue-100 text-blue-700': product.access_tier?.includes('free'),
                        'bg-emerald-100 text-emerald-700': !product.access_tier || product.access_tier === 'all',
                        'bg-indigo-100 text-indigo-700': !['gold', 'silver', 'free', 'all', ''].some(k => product.access_tier?.includes(k))
                      }">
                  {{ product.access_tier ? product.access_tier.replace(/-/g, ' ') : 'ALL' }}
                </span>
              </td>
      ...
      <div class="bg-white w-full max-w-4xl max-h-[90vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-fade-in">
        ...
        <div class="p-5 border-b border-slate-200 bg-slate-50 flex flex-col gap-4">
          <div class="flex justify-between items-center">
            <h2 class="text-lg font-black text-slate-800">{{ isEditMode ? 'Edit Data' : 'Tambah Data Baru' }}</h2>
            <button @click="closeModal" class="text-slate-400 hover:text-red-500 transition text-xl font-bold">&times;</button>
          </div>
          
          <div>
            <label class="block text-[12px] font-black text-slate-800 mb-1">Jenis Produk <span class="text-red-500">*</span></label>
            <select v-model="form.type" disabled class="w-full border border-slate-300 rounded-lg px-4 py-2.5 text-sm font-bold bg-slate-100 text-slate-500 cursor-not-allowed outline-none shadow-sm">
            ...
            <p class="text-[11px] text-blue-600 mt-1.5 font-bold">Jenis produk otomatis dikunci menyesuaikan menu yang Anda buka saat ini.</p>
          </div>
        </div>
            ...
            <div v-show="activeTab === 'Dasar'" class="space-y-4 animate-fade-in">
              ...
              <div class="p-4 rounded-lg bg-blue-50/50 border border-blue-100 space-y-4">
                ...
                <div v-if="form.type === 'Software'">
                  <label class="block text-[12px] font-black text-slate-800 mb-1">Pilih Paket Akses (Khusus Gold) <span class="text-red-500">*</span></label>
                  <select v-model="form.access_tier" @change="handleTierChange" required class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm font-bold text-amber-700 bg-amber-50 outline-none focus:border-amber-500">
                    <option value="" disabled>-- Pilih Variasi Paket Gold --</option>
                    <option v-for="tier in goldTiers" :key="tier.slug" :value="tier.slug">
                      {{ tier.name }} (Durasi: {{ tier.duration_in_months > 0 ? tier.duration_in_months + ' Bulan' : 'Lifetime' }}) - Rp {{ Number(tier.price).toLocaleString('id-ID') }}
                    </option>
                  </select>
                  <p class="text-[10px] text-slate-500 mt-1.5 leading-snug">Harga produk di bawah akan otomatis terisi sesuai dengan harga paket langganan Gold yang Anda pilih.</p>
                </div>
                ...
                <div v-if="form.type === 'Digital'">
                  ...
                  <select v-model="form.access_tier" @change="handleTierChange" required class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-blue-500 bg-white">
                    ...
                    <option v-for="tier in masterTiers" :key="tier.slug" :value="tier.slug">{{ tier.name }}</option>
                    <option value="all">Bisa Diakses Semua (Termasuk Non-Member)</option>
                  </select>
                  <p class="text-[10px] text-slate-500 mt-1">Pilih Master Tier terendah yang diperbolehkan mengakses produk ini. Tier di atasnya otomatis dapat akses.</p>
                  <div class="mt-4 border-t border-blue-100 pt-4">
                    ...
                    <input type="file" @change="handleDigitalUpload" class="w-full text-xs file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:font-bold file:bg-blue-600 file:text-white hover:file:bg-blue-700 border border-slate-200 rounded-lg bg-white p-1">
              ...
              <div class="flex gap-4 mt-4">
                <div class="flex-1">
                  <label class="block text-[12px] font-black text-slate-800 mb-1">Harga Pesan Produk (Rp) <span class="text-red-500">*</span></label>
...
form::-webkit-scrollbar-thumb { background-color: #cbd5e1; border-radius: 10px; }
</style>