...
const menuItems = [
  { name: 'Dashboard', icon: '📊', route: '/dashboard/customer' },
...
const handleLogout = async () => {
  try {
    ...
    // Hapus sesi lokal dan kembalikan ke halaman depan publik
    localStorage.clear();
    ...
    <aside class="w-64 bg-slate-900 text-slate-300 flex flex-col border-r border-slate-850 hidden md:flex">
      ...
      <nav class="flex-1 p-4 space-y-1.5">
        <router-link 
          ...
          :key="item.name" 
          :to="item.route"
          exact-active-class="bg-slate-800 text-white font-bold"
          class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm hover:bg-slate-800 hover:text-white transition group"
        >
      ...
      <header class="h-16 bg-white border-b border-slate-200 px-6 flex items-center justify-between shadow-sm">
        ...
        <div class="flex items-center gap-3">
          <div class="text-right">
            ...
            <p class="text-[11px] text-slate-400 font-bold uppercase">Client Account</p>
          </div>
          <div class="w-10 h-10 bg-[#B48440]/10 text-[#B48440] font-black rounded-xl flex items-center justify-center border border-[#B48440]/20">
